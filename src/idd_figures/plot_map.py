...
def plot_base_admins(ax_map, plot_dict):
    ...
    admin0_polygons.plot(ax=ax_map, color='lightgrey', edgecolor='black', linewidth=0, transform=ccrs.PlateCarree())
    if map_dict.get('plot_admin1s', False):
        ...
        admin1s_to_plot.boundary.plot(ax=ax_map, color='darkgrey', linewidth=0.25, transform=ccrs.PlateCarree())
...
def plot_data_admins(ax_map, plot_dict, linewidth=0):
    ...
    admin2_with_data.plot(column=data_dict['data_column'], ax=ax_map, cmap=plot_dict['bin_dict']['cmap'], norm=plot_dict['bin_dict']['norm'], legend=False, edgecolor=None, linewidth=linewidth, transform=ccrs.PlateCarree())
    admin0_polygons = plot_dict['map_dict']['admin0_polygons']
    admin0_polygons.boundary.plot(ax=ax_map, color='black', linewidth=0.5, transform=ccrs.PlateCarree())
...
def add_colorbar(fig, ax, plot_dict):
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, orientation='horizontal', shrink=legend_dict['color_bar_dict']['shrink'], pad=legend_dict['color_bar_dict']['pad'], aspect=legend_dict['color_bar_dict']['aspect'], fraction=legend_dict['color_bar_dict']['fraction'], ticks=bin_centers)
