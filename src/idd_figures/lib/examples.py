...
from idd_figures.lib.layouts.grids import cell, facet_grid, grid, label, legend, paint, panel_grid
...
def exemplar_forecast_all_plus_each(
    panel_data=None, *, group="Global", measure="mort", metric="count"
):
    ...
    cells = [
        cell(
            (0, 0),
            paint(lines_panel, base, **FORECAST_LINES, value_scale=vscale, show_ci=False),
            name="all",
            title="All SSPs",
        )
    ]
    for k, s in enumerate(ssps):
        ...
        cells.append(
            cell(
                (i, j),
                paint(lines_panel, sub, **FORECAST_LINES, value_scale=vscale, show_ci=True),
                title=s,
                sharex="all",
                sharey="all",
            )
...
def exemplar_scatter(df=None):
    ...
    ax = plot_scatter(
        ...
        x="x",
        y="y",
        size=("weight", size_by_logpop),
        color="C1",
        alpha=0.5,
        ref_lines={"h": 0, "v": prog["x"].mean()},
        xlabel="value (start year)",
        ylabel="AROC",
        title="made-up AROC scatter",
    )
...
def exemplar_left_behind(df=None):
    ...
    ax = plot_scatter(
        ...
        ref_lines={"h": 0, "v": 0},
        xlabel="Δ value from national",
        ylabel="Δ AROC from national",
        ...
        for j, g in enumerate(groups):
            ...
            for r, metric in enumerate(["count", "rate"]):
                ...
                inner.append(
                    cell(
                        (r, 0),
                        paint(lines_panel, d, **FORECAST_LINES, value_scale=vs, show_ci=False),
...
def exemplar_composition(df=None):
    ...
    return plot_composition(
        df,
        components=("health_index", "education_index", "income_index"),
        color_col="hdi",
        color_label="HDI",
...
def exemplar_map_panel(*, draw_data=True):
    ...
    return map_panel(
        ...
        boundary_gdf=gdf,
        ocean=False,
        title="Made-up choropleth — map_panel demo",
        legend_title="value",
        panel_letter="A",
        fig_width=8,
        draw_data=draw_data,
...
def _world_map(kind, *, continuous, ocean=True):
    ...
    if continuous:
        ...
        return map_panel(
            ...
            mappable=sm,
            use_colorbar=True,
            title=f"World {kind} — continuous ramp",
            **content,
            **common,
        )
    ...
    return map_panel(
        ...
        norm=norm,
        colors=colors,
        bin_labels=map_bin_labels(bins, ge=True),
        title=f"World {kind} — binned",
        **content,
        **common,
    )
