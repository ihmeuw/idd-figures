...
def guide_grid_anatomy(
    *,
    ...
    """
    margins = (
        {"left": 0.10, "right": 0.94, "top": 0.88, "bottom": 0.14} if margins is None else margins
    )
    ...
    _arrow(
        fig, (0.0, ymid), (a.x0, ymid), f'margins["left"] = {margins["left"]}', dx=0.075, dy=0.03
    )
    _arrow(
        fig, (b.x1, ymid), (1.0, ymid), f'margins["right"] = {margins["right"]}', dx=-0.075, dy=0.03
    )
    _arrow(fig, (xmid, a.y1), (xmid, 1.0), f'margins["top"] = {margins["top"]}', dx=0.14)
    _arrow(
        fig, (0.70, 0.0), (0.70, c.y0), f'margins["bottom"] = {margins["bottom"]}', dx=0.0, dy=0.035
    )
    ...
    if notes:
        _note(
            fig,
            0.045,
            "Ratios split the area INSIDE the margins; gaps are charged against the\n"
            "MEAN panel size (geometry.gap_factor inverts this).",
            fontsize=8,
        )
        _note(
            fig,
            0.005,
            "Nested grids have NO margins of their own — the parent's cell IS the\n"
            "margin control (spacer rows/cols if needed).",
            fontsize=8,
        )
    if cell_boxes:
        _outline_cells(fig)
...
def guide_map_panel_anatomy(
    *,
    ...
    fig.text(
        0.05,
        map_p.y0 + 0.02,
        'aspect is EXPLICIT (set_aspect(1.0, "box")):\n'
    ...
    *,
    ...
    fig = map_facet(
        [
            {
                "panels": [
                    dict(p, title="A", base_admin_gdf=gdf),
                    dict(p, title="B", base_admin_gdf=gdf),
            ...
            {
                "panels": [dict(p, title="D", base_admin_gdf=gdf)],
                "extent": SYNTHETIC_EXTENT,
                "cbar": None,
            },
        ],
    ...
    _arrow(
        ...
        (xa, m0.y1),
        (xa, 1.0),
        f'panel_title_h = {panel_title_h}" (top pad)',
        dx=0.27,
        fontsize=8,
    )
    _arrow(
        ...
        (xa, bar.y1),
        (xa, m0.y0),
        f'gap = panel_title_h = {panel_title_h}"',
        dx=0.12,
        fontsize=8,
    )
    _arrow(
...
def guide_bar_cell_anatomy(
    *, margin=0.06, spacing=0.03, bin_bottom=0.50, bin_top=0.90, label_gap=0.10, cell_boxes=True
):
    ...
    _cmap, _norm, colors = binned_colormap([0, 25, 50, 75, 100], base_cmap="RdYlBu_r")
    ...
    spec = grid(
        ...
        margins={"left": 0.03, "right": 0.985, "top": 0.86, "bottom": 0.22},
        wspace=0.08,
    )
    ...
    _arrow(
        fig,
        (fx(r0.get_x() + r0.get_width()), fy(0.7)),
        (fx(r1.get_x()), fy(0.7)),
        f"spacing = {kn['spacing']}",
        dy=0.08,
        fontsize=8,
    )
    _arrow(
        fig,
        (fx(1.02) - 0.005, fy(kn["bin_bottom"])),
        (fx(1.02) - 0.005, fy(kn["bin_top"])),
    ...
    _arrow(
        fig,
        (fx(r0.get_x() + r0.get_width() / 2), fy(kn["bin_bottom"])),
        (fx(r0.get_x() + r0.get_width() / 2), fy(kn["bin_bottom"] - kn["label_gap"])),
        f"label_gap = {kn['label_gap']}",
        dx=0.10,
        fontsize=8,
    )
    _note(
        fig,
        0.03,
        "All fractions are OF THE CELL: the cell's grid span is the footprint; these "
...
def guide_text_placement(*, margins=None, notes=True, cell_boxes=True):
    ...
    margins = (
        {"left": 0.14, "right": 0.95, "top": 0.86, "bottom": 0.30} if margins is None else margins
    )
    ...
    def painter(ax, d):
        lines_panel(
            ...
            value="value",
            hue="series",
            xlabel="xlabel (hangs below ticks)",
            ylabel="ylabel (hangs left)",
        )
        return ax

    spec = grid(
        (1, 1),
        [cell((0, 0), paint(painter, df), name="p", title="title (hangs above)")],
        margins=margins,
    )
    ...
    _arrow(
        fig, (0.28, p.y1), (0.28, 1.0), f'margins["top"] = {margins["top"]}', dx=0.155, fontsize=8
    )
    _arrow(
        fig,
        (0.80, 0.0),
        (0.80, p.y0),
        f'margins["bottom"] = {margins["bottom"]}',
        dx=0.0,
        dy=0.0,
        fontsize=8,
    )
    ...
    if notes:
        _note(
            fig,
            0.125,
            "The grid solver places BOXES (grey panel region). Titles, tick labels and\n"
        ...
        _note(
            fig,
            0.068,
            "above: panel_title_h (map_facet) / title_h (map_panel);  below: bar cells\n"
        ...
        _note(
            fig,
            0.01,
            "Text extents exist only after a draw -> the coming assert_no_clipping(fig)\n"
            "guard is post-draw by necessity.",
            fontsize=8,
        )
    if cell_boxes:
        _outline_cells(fig)
...
def guide_text_styles(
    *,
    ...
    spec = grid(
        (1, 1),
        [cell((0, 0), paint(painter, df), name="p")],
    ...
    _note(
        fig,
        0.015,
        "No single per-region 'font' knob: every text object has an owner-setter.\n"
        "Wholesale defaults (family, base size) go through mpl.rc_context({...}) "
        "around figure creation.",
        fontsize=8,
    )
    if cell_boxes:
        _outline_cells(fig)
...
def guide_coordinate_frames(*, letter_xy=(0.04, 0.92), cell_boxes=True):
    ...
    spec = grid(
        ...
        [
            cell(
                ...
                paint(_empty, None),
                name="ax_frame_0",
                title="ax.text(..., transform=ax.transAxes)",
            ),
            cell(
                (0, 1),
                paint(_empty, None),
                name="fig_frame_0",
                title="fig.text(x, y)  [figure coords]",
            ),
            cell((1, 0), paint(_empty, None), name="ax_frame_1"),
            cell((1, 1), paint(_empty, None), name="fig_frame_1"),
        ],
        height_ratios=[1, 2],
        margins={"left": 0.06, "right": 0.97, "top": 0.90, "bottom": 0.16},
        wspace=0.15,
        hspace=0.55,
    )
    ...
    fig.text(
        fx + 0.035,
        fy - 0.004,
        f"({fx:.3f}, {fy:.3f}) in figure coords",
        va="top",
        fontsize=8,
        color=_INK,
    )
    ...
    _note(
        fig,
        0.055,
        "One physical point, many addresses: transData glues to the data;\n"
    ...
    _note(
        fig,
        0.008,
        "All frames interconvert exactly (ax.transAxes.transform ->\n"
