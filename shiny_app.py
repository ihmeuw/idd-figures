...
continent_options = {c: c for c in continents.keys()}
...
def plot_countries_in_continent_cartopy(continent_name):
    ...
    for idx, (rec, val) in enumerate(zip(records, country_values)):
        ...
        ax.add_patch(plt.Polygon(list(geom.exterior.coords), facecolor=color, edgecolor='black', linewidth=2, zorder=2))
        # Partition country into contiguous fake states
        admin1s = recursive_split(geom, depth=0, max_depth=4)
        for admin1 in admin1s:
            if admin1.area < 0.01: continue
            state_val = np.random.rand() * 100
            state_bin_idx = np.digitize(state_val, bins) - 1
            state_color = cmap(state_bin_idx / (len(bins)-2))
            try:
                ax.add_patch(plt.Polygon(list(admin1.exterior.coords), facecolor=state_color, edgecolor='white', linewidth=1, zorder=3))
            except Exception:
                continue
        # Label country
        centroid = geom.centroid
        ax.text(centroid.x, centroid.y, rec.attributes.get('NAME', f"Country {idx+1}"), ha='center', va='center', fontsize=10, color='black', zorder=4)
